...
def if_then_else(cond_input, 
	output_1, 
	output_2):
	return output_1 if cond_input else output_2
...
class GP_model(object):
	...
	def __init__(self, 
		...
		self.mdl = None
		self.threshold = 0.5 #0.5#$0. -> should not be changed either for with and without sigmoid

		# for dist
		self.opt_prob_dist = None
		self.use_min_max_scaler = use_min_max_scaler
		self.sel_wo_dup = False

		# currenlty under test
		self.dp_scores = None


	def set_dp_scores(self, dp_scores):
		"""
		"""
		self.dp_scores = dp_scores
	...
	def predict_probas(self,
		...
		print (individual_in_exec_form)
		#compute suspicousness score for each data row(per method) in current_data
		for i in range(features.shape[0]):
			pred_probas[i] = eval(individual_in_exec_form)

		##### will add softmax here -> to calculate softmax, the gp should have two return values, i.e., one for each class
		#pred_probas_aft_softmax = softmax()
		######

		# apply sigmoid here
		#original_pred_probas = np.zeros(pred_probas.shape)
		#original_pred_probas[:] = pred_probas[:]
		#print ("Before", np.min(pred_probas), np.max(pred_probas))
		pred_probas = expit(pred_probas)
		#print ("After", np.min(pred_probas), np.max(pred_probas))
		#per_sigmoid_pbs = [np.float32(expit(pb)) for i,pb in enumerate(original_pred_probas)]
		#is_different = [pb_1 != pb_2  for pb_1,pb_2 in zip(per_sigmoid_pbs, pred_probas)]
		#print (any(is_different))
		#indices_to_diff = [i for i,flag in enumerate(is_different) if flag]
		#print (len(indices_to_diff))
		#print (len(is_different))
		#print ([list(zip(per_sigmoid_pbs, pred_probas))[idx] for idx in indices_to_diff])
		#sys.exit()
		...
		###
		if self.dp_scores is not None:
			print ("==========IN here==============")
			print ("\t++", np.min(self.dp_scores), np.max(self.dp_scores), np.mean(self.dp_scores),)
			pred_probas = 0.8 * self.dp_scores + 0.2 * pred_probas
		###
...
#		opt_prob_dist = self.scale(opt_prob_dist)
	...
	def redef_brier_score(self,
		pred_probas_in_class_1,
		pred_probas_in_class_0):
		"""
		self.threshold = 
		"""
		# this function assumen self.threshold to be 0.0

		assert self.threshold == 0., self.threshold

		bs_class_1 = np.sum((pred_probas_in_class_1.clip(max = 1.)-1.)**2)
		bs_class_0 = np.sum((pred_probas_in_class_0.clip(min = -1.)-(-1.))**2)
		bs = bs_class_1 + bs_class_0
	...
	def eval_func(self, individual):
		...
		#print (",".join(["{0:.3f}".format(v) for v in [np.min(pred_probas), np.max(pred_probas), np.mean(pred_probas), np.median(pred_probas)]]))
		#sys.exit()
		...
		if self.use_dist_as_fit:
			...
			auc = np.float32(roc_auc_score(self.labels, pred_probas.clip(0.,1.)))
			mse = np.sum((self.labels - pred_probas)**2)
			...
			from sklearn.metrics import f1_score, \
				accuracy_score, balanced_accuracy_score, average_precision_score, roc_auc_score, log_loss#, log_loss #confustion_matrix, f1_score	
			#print (pred_probas)
			#print (predictions)
			...
			#	labels = [0,1])
			if self.is_pareto:
				# use f1 and loss (brier loss or l2-penality)
				# instead of ROC-AUC, we choose to use F1 score as our data is imbalanced
				# and we proritise th postive class over negative class
				# + PR_AUC, which can be computed by average_precision_score, can be used instead of f1
				...
				#AP = np.float32(average_precision_score(self.labels, pred_probas))
				loss = np.sum((self.labels - pred_probas)**2)
				#bal_acc = balanced_accuracy_score(self.labels, predictions)
				#loss = np.float32(accuracy_score(self.labels, predictions))
				#loss = log_loss(self.labels, pred_probas, eps = 1e-7)
				#in_class_1 = pred_probas[self.labels == 1]
				#in_class_0 = pred_probas[self.labels == 0]

				#bs = np.float32(self.redef_brier_score(in_class_1, in_class_0))
				#print (pred_probas)
				#auc = np.float32(roc_auc_score(self.labels, pred_probas))
				fitness = (f1, -loss)
				#fitness = (bal_acc, -loss)
				#fitness = (AP, -loss)
			else:
	...
	def selTournament_mo_vers(self,
		...
		while len(winners) < k:
		#for _ in range(k):
			if len(remained_individuals) <= tournsize:
				tournsize = len(remained_individuals)

			sampled_inds = tools.selRandom(remained_individuals, tournsize)
			selected = tools.selNSGA2(sampled_inds, 1)[0]
			#print (selected)
			if self.sel_wo_dup:
				indices_to_remained = [idx for idx,ind_in_str in enumerate(remained_individuals_in_str) if ind_in_str!=str(selected)]
				remained_individuals = [remained_individuals[idx] for idx in indices_to_remained]
				remained_individuals_in_str = [remained_individuals_in_str[idx] for idx in indices_to_remained]
				#for idx_to_selected in indices_to_selected:
				#:qprint ("===selected({}): {}".format(len(winners), str(selected)))
				#	remained_individuals.pop(idx_to_selected)
				#	remained_individuals_in_str.pop(idx_to_selected)

			print ("--", len(individuals), len(remained_individuals))
			#print ([str(ind) for ind in remained_individuals])
	...
	def gen_primitives(self, headers):
		...
		pset = gp.PrimitiveSetTyped('main', [float] * len(headers), float) # arity = len(headers)
		...
		#pset.addPrimitive(if_then_else, 2, name = "if_then_else")

		pset.addPrimitive(np.add, [float,float], float, name = "np.add")
		pset.addPrimitive(np.subtract, [float,float], float, name = "np.subtract")
		pset.addPrimitive(np.multiply, [float,float], float, name = "np.multiply")
		pset.addPrimitive(np.negative, [float], float, name = "np.negative")
		pset.addPrimitive(protect_div, [float, float], float, name = "protect_div")
		pset.addPrimitive(protect_sqrt, [float], float, name = "protect_sqrt")
		# if-then-else
		#pset.addPrimitive(operator.lt, [float, float], bool, name = "operator.lt")
		#pset.addPrimitive(operator.eq, [float, float], bool, name = "operator.eq")
		#pset.addPrimitive(if_then_else, [bool, float, float], float, name = "if_then_else")

		#pset.addTerminal(1.0) # vary ..?
		pset.addTerminal(1.0, float) # vary ..?
...
#	def ind_in_leaner_form(self, expr):
#		"""
#		"""
#		np.add(str(expr)
#		gp.PrimitiveTree(operator.mul)
#		pass
...
#		print (pset)
		...
		for idx_to_gen in tqdm(range(1, ngen + 1)):
			...
			while len(next_pop) < num_pop:# select a pair of individuals that will become the parent
				...
				if self.check_duplicate(parents[0], parents[1]) and self.sel_wo_dup:
					offsprings = algorithms.varAnd(parents, toolbox, 0.0, mutpb + cxpb)	
				else:		
			...
			for ind_1, ind_2 in zip(pop[::2], pop[1::2]):
				num_same += int(self.check_duplicate(ind_1, ind_2))
	...
	def fit_model(self,
		...
		if not self.is_pareto:
			...
			# maybe add another feature here ... for the case of more than one?
	...
	def min_max_scale(self, vs):
		"""
		MinMax Scale: here max_v = 1.0, and min_v = -1.0
		+ Clipping
		"""
		MIN_V = -1.; MAX_V = 1.

		scaled = ((vs - MIN_V)/(MAX_V - MIN_V)).clip(0.,1.) 
	...
	def predict(self, 
		...
		if predtype == 'label':
			...
			from sklearn.preprocessing import MinMaxScaler
			predictions = self.predict_probas(features, str(self.mdl), with_scaled = False)#True)
			print ("Before clipping Predictions", np.min(predictions), np.max(predictions), np.mean(predictions), np.median(predictions))
			predictions = predictions.clip(0., 1.)
			print ("After clipping Predictions", np.min(predictions), np.max(predictions), np.mean(predictions), np.median(predictions))
