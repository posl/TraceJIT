...
#		opt_prob_dist = self.scale(opt_prob_dist)
	...
	def eval_func(self, individual):
		...
		"""
		indices_to_selected = np.random.choice(len(self.labels), size = 200 if len(self.labels) > 200 else len(self.labels))
		pred_probas = self.predict_probas(self.features[indices_to_selected], str(individual))
		...
		if self.use_dist_as_fit:
			...
			from sklearn.metrics import f1_score, accuracy_score, roc_auc_score, log_loss#, log_loss #confustion_matrix, f1_score	
			#print (pred_probas)
			#print (predictions)
			...
			#	labels = [0,1])
			if self.is_pareto:
				f1 = np.float32(f1_score(self.labels[indices_to_selected], predictions, average = 'binary')) # macro, binary, micro, weighted 
				#acc = np.float32(accuracy_score(self.labels, predictions))
				#pred_probas = self.scale(pred_probas)
				
				loss = np.sum((self.labels[indices_to_selected] - pred_probas)**2)
